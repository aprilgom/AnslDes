...
- [ ] `equal-icon-feature-columns`: 동일한 세 feature column과 icon-above-heading anatomy 결합을 검출한다.
- [ ] `full-viewport-centered-hero`: full viewport minimum height와 양축 중심 정렬 hero 결합을 검출한다.
...
- [ ] data grid와 의도적인 immersive canvas를 Hallmark supplement의 negative fixture로 보존한다.

## 완료 조건

- [ ] 14개 rule의 browser/native/design-document 적용 가능성 표가 완성된다.
