...
- [ ] `unverified-social-proof`: metric, testimonial과 logo claim의 source reference 누락을 검출한다.
...
- [ ] claim truth를 lint가 추론하지 않고 consumer content registry의 stable source reference만 검증한다.

## 완료 조건

- [ ] 여섯 rule의 한국어 적용·비적용 기준이 예문과 함께 고정된다.
- [ ] advisory가 silent pass나 blocking error로 임의 변환되지 않는다.
- [ ] content registry 미실행은 `not-run`이며 proof가 없는 claim을 pass로 표시하지 않는다.
