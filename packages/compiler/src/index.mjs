...
export const designContextGenerator = {
  id: "ansldes-design-context",
  version: "1.0.0",
};

export function generateDesignContext(definition) {
  validateDefinition(definition);
  const sourceContractSha256 = fingerprint(definition);
  const typography = definition.foundations.typography;
  const sidecar = canonicalize({
    $schema: "https://ansldes.dev/schema/design-context.v1.json",
    schemaVersion: 1,
    generator: designContextGenerator,
    source: {
    ...
    typography: {
      familyName: typography.familyName,
      weights: typography.weights,
      tokens: typography.tokens,
      roles: typography.roles,
    },
    colors: definition.foundations.color,
    radii: definition.foundations.radius,
  });
  const quote = (value) => JSON.stringify(String(value));
  const lines = [
    "<!-- Generated by ansldes-design-context@1.0.0; DO NOT EDIT. -->",
    "---",
    `generator: ${designContextGenerator.id}`,
    `generatorVersion: ${designContextGenerator.version}`,
    `sourceContractSha256: ${sourceContractSha256}`,
    "typography:",
    `  familyName: ${quote(typography.familyName)}`,
    "  scale:",
  ];
  for (const [name, token] of Object.entries(typography.tokens).sort(
    ([left], [right]) => left.localeCompare(right),
  )) {
    lines.push(`    ${quote(name)}: ${quote(`${token.fontSize}px`)}`);
  }
  lines.push("colors:");
  for (const [name, value] of Object.entries(
    definition.foundations.color.primitive,
  ).sort(([left], [right]) => left.localeCompare(right))) {
    lines.push(`  ${quote(name)}: ${quote(value)}`);
  }
  for (const [name, value] of Object.entries(
    definition.foundations.color.asset,
  ).sort(([left], [right]) => left.localeCompare(right))) {
    lines.push(`  ${quote(`asset.${name}`)}: ${quote(value)}`);
  }
  lines.push("rounded:");
  for (const [name, value] of Object.entries(
    definition.foundations.radius.primitive,
  ).sort(([left], [right]) => left.localeCompare(right))) {
    lines.push(`  ${quote(name)}: ${quote(`${value}px`)}`);
  }
  lines.push(
    "---",
    "",
    `# ${definition.id} design context`,
    "",
    "This file is generated from the canonical AnslDes definition. Do not copy token lists into it.",
    "",
  );
  return { designMarkdown: lines.join("\n"), sidecar };
}
...
export function compileDesignSystem(definition) {
  ...
  const content = {
    ...
    definition: {
      ...
      ...(definition.colorUsage
        ? { colorUsage: canonicalize(definition.colorUsage) }
        : {}),
    },
    themes,
  };
...
export function validateDefinition(definition) {
  ...
  validateMotionRegistry(definition, diagnostics);
...
function validateThemes(definition, diagnostics) {
  ...
  for (const [id, palette] of Object.entries(
    definition.colorUsage?.approvedPalettes ?? {},
  )) {
    for (const theme of palette.themes ?? []) {
      if (!names.includes(theme)) {
        diagnostics.push(
          `approved palette ${id} references unknown theme ${theme}`,
        );
      }
    }
  }
...
function validateMotionRegistry(definition, diagnostics) {
  for (const [id, transition] of Object.entries(
    definition.foundations?.motion?.transitions ?? {},
  )) {
    if (
      (transition.owner === undefined) !==
      (transition.purpose === undefined)
    ) {
      diagnostics.push(
        `motion transition ${id} must declare owner and purpose together`,
      );
    }
  }
}
